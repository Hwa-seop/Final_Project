#!/usr/bin/env python3
"""
Main application for Unified ROI Tracking with Helmet Detection.

This script provides a complete system that combines helmet detection and ROI-based
tracking using DeepSORT. It can detect helmets and monitor when people enter
danger zones.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Unified ROI Tracking with Helmet Detection')
    ...
    parser.add_argument('--iou', '-i', type=float, default=0.1,
    ...
    parser.add_argument('--detection-interval', type=int, default=1,
                       help='Run detection every N frames (1 = every frame, higher = faster)')
    ...
    parser.add_argument('--skip-roi-setup', action='store_true',
                       help='Skip ROI setup (use default or programmatic ROI)')
...
def main():
    ...
    # Initialize unified ROI tracker
    ...
    try:
        ...
        print("UnifiedROITracker initialized successfully!")
    except Exception as e:
        print(f"Error initializing UnifiedROITracker: {e}")
        cap.release()
        sys.exit(1)
    
    # ROI setup
    if not args.skip_roi_setup:
        ...
        # Set a default ROI (center rectangle)
        ret, frame = cap.read()
        if ret:
            ...
            tracker.set_roi(default_roi)
            print("Using default ROI (center rectangle)")
        else:
            print("Error: Could not read frame for default ROI")
            cap.release()
            sys.exit(1)
    ...
    print("Starting unified ROI tracking with helmet detection...")
            ...
            if not paused:
                ...
                if processed_frame is not None:
                    ...
                    stats_text = [
                        ...
                        f"In Danger Zone: {frame_stats.get('people_in_danger_zone', 0)}",

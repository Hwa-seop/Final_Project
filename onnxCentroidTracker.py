...
# ----------------------------
# PyTorch 기반 YOLOv5 모델 로드 및 추론 함수 정의
# ----------------------------
model = torch.hub.load('ultralytics/yolov5', 'yolov5n')  # yolov5n 모델 로딩
...
while True:
    ...
    if frame_count % 3 == 0:  # 매 3프레임마다 YOLO 추론
        ...
        for det in dets:
            ...
            if conf < 0.5 or int(cls) != 0:  # 사람 클래스가 아니면 무시
                continue
            w, h = x2 - x1, y2 - y1
    ...
    objects = tracker.update(last_boxes)  # 추적기 업데이트
    ...
    cv2.imshow("YOLOv5 PT + Centroid", frame)  # 결과 화면 표시

...
from http_helmet import HTTPHelmetController  # HTTP 헬멧 제어 모듈 추가
...
def initialize_helmet_controller():
    ...
    try:
        # 설정 파일에서 ESP8266 설정을 자동으로 가져옴
        helmet_controller = HTTPHelmetController()
        ...
        helmet_controller.start_connection_monitor()
        
        print("✅ HTTP 헬멧 컨트롤러 초기화 완료")
        return True
...
def update_statistics_for_id(track_id, has_helmet, in_danger_zone):
    ...
    if track_id not in processed_ids:
        ...
        if helmet_controller:
            if not has_helmet or in_danger_zone:
...
def shutdown_server():
    ...
    if helmet_controller:
        helmet_controller.stop_connection_monitor()
